...
CREATE TABLE IF NOT EXISTS `gestion_tickets`.`tickets` ( `TICKETID` SMALLINT NOT NULL AUTO_INCREMENT, `REQUESTID` SMALLINT NOT NULL, `PROJECTID` SMALLINT NOT NULL, `ID_PROGRAMADOR` SMALLINT NOT NULL, `ID_TESTER` SMALLINT NOT NULL, `TICKET_STATUS` VARCHAR(16) NOT NULL, `INTERNALCODE` VARCHAR(24) NOT NULL, `STARTDATE` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `ENDDATE` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`TICKETID`, `REQUESTID`), CONSTRAINT `fk_tickets_requests1` FOREIGN KEY (`REQUESTID`) REFERENCES `gestion_tickets`.`requests` (`REQUESTID`) ON DELETE CASCADE ON UPDATE NO ACTION) ENGINE = InnoDB AUTO_INCREMENT = 0